...
def fix_pk(grad, xk, gk, pk, scale=0.1, tol=1e-5):
    # estimate vk
    u1 = gk / np.linalg.norm(gk) if np.linalg.norm(gk) > 0 else gk

    gp = grad(xk + pk)
    eps = tol / np.linalg.norm(gp)
    u2 = (grad(xk + eps * gp) - gk) / eps
    u2 = u2 / np.linalg.norm(u2) if np.linalg.norm(u2) > 0 else u2

    vk = (u1 + u2) / np.linalg.norm(u1 + u2) if np.linalg.norm(u1 + u2) > 0 else (u1 + u2)

    # estimate alpha
    if np.linalg.norm(vk) == 0:
        return pk

    # u3 = gk - gp  # TODO: delete this line

    eps = tol / np.linalg.norm(vk)
    bv = (grad(xk + eps * vk) - gk) / eps

    alpha = np.inner(gp, bv) / np.inner(bv, bv)
    # if np.linalg.norm(gk) < 1:
    #     print(
    #         # alpha,
    #         # np.inner(gk, gp) / np.linalg.norm(gk) / np.linalg.norm(gp),
    #         np.inner(u1, u2) / np.linalg.norm(u1) / np.linalg.norm(u2),
    #         # np.inner(gk, u3) / np.linalg.norm(gk) / np.linalg.norm(u3),
    #         np.linalg.norm(gk),
    #         # np.linalg.norm(pk),
    #         # np.linalg.norm(gp),
    #         # np.linalg.norm(bv),
    #     )
    # if np.linalg.norm(gp) / np.linalg.norm(gk) > 1:
    #     # alpha = 0.1 * np.abs(np.inner(gk, pk) / np.inner(gk, vk))
    #     alpha = 0.001
    #     # alpha = 0.1 * np.linalg.norm(pk)
    # else:
    #     alpha = max(alpha, scale * tol)
    alpha = max(min(alpha, scale), scale * tol)

    return pk - alpha * vk


def fast_bfgs(func, grad, x0, m=8, secant=False, scale=0.1, tol=1e-5, max_iters=1000):
    xk = x0
    gk = grad(xk)
    logs = [[*xk, func(xk), np.linalg.norm(gk)]]
    ...
    while True:
        ...
        if m < x0.__len__() or sks.__len__() <= x0.__len__():
            # fix pk
            pk = fix_pk(grad, xk, gk, pk, scale=scale, tol=tol)
...
#  --------------------------- adaptive fast-bfgs ---------------------------
def increase_lk(yk, sks, lk):
    current_dim = sks.__len__() - 1

    ys = np.array([[-np.inner(yk, sk) for sk in sks[:-1]]])
    tk = np.vstack((np.eye(current_dim), ys))
    lk = tk @ lk @ tk.T
    lk[-1, -1] += 1
    return lk


def keep_lk(yk, sks, lk):
    current_dim = sks.__len__() - 1

    tk = np.zeros(shape=(current_dim, current_dim))
    tk[:-1, 1:] = np.eye(current_dim - 1)
    tk[-1] = np.array([-np.inner(yk, sk) for sk in sks[:-1]])
...
def decrease_lk(yk, sks, lk):
    current_dim = sks.__len__() - 1

    lk.__len__()
    tk = np.zeros(shape=(current_dim - 1, current_dim))
    tk[:-1, 2:] = np.eye(current_dim - 2)
    tk[-1] = np.array([-np.inner(yk, sk) for sk in sks[:-1]])

    mat = np.array(sks[2:]) @ np.array(sks[2:]).T
    rhs = np.array(sks[2:]) @ np.array(sks[:2]).T
...
def adaptive_fast_bfgs(func, grad, x0, scale=0.1, min_m=4, max_m=8, tol=1e-5, max_iters=1000):
    xk = x0
    gk = grad(xk)
    logs = [[*xk, func(xk), np.linalg.norm(gk)]]
    ...
    while True:
        ...
        norm_u1 = u1 / np.linalg.norm(u1) if np.linalg.norm(u1) > 0 else np.zeros_like(u1)
        norm_u2 = u2 / np.linalg.norm(u2) if np.linalg.norm(u2) > 0 else np.zeros_like(u2)

        vk = norm_u1 + norm_u2
        vk = vk / np.linalg.norm(vk) if np.linalg.norm(vk) > 0 else vk
        ...
        if np.linalg.norm(vk) == 0:
            ...
            alpha = np.inner(gp, bv) / np.inner(bv, bv)
            # print(alpha, np.linalg.norm(pk))
            alpha = max(min(alpha, scale), scale * tol)

        flag = (alpha / np.linalg.norm(pk) <= scale) and alpha < scale
        pk = pk - alpha * vk
        # if not flag:
        ...
        if (k < max_m) or (flag and sks.__len__() <= max_m):
            lk = increase_lk(yk, sks, lk)
            if sks.__len__() > max_m:
                sks = sks[1:]
        elif (not flag) and sks.__len__() - 1 > min_m:
            lk = decrease_lk(yk, sks, lk)
            sks = sks[2:]
        else:
            lk = keep_lk(yk, sks, lk)
            sks = sks[1:]

        k += 1
        # print(alpha, flag, func(xk))
